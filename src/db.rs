...
impl PsInterface {
    pub async fn new() -> Result<PsInterface, sqlx::Error> {
        let opt = PgConnectOptions::new()
            ...
            .username(
                &std::env::var("PS_USERNAME")
                    .expect("a PS_USERNAME enviroment variable is expected"),
            )
            .database("records_ps");
        ...
        T: Bindable + for<'r> FromRow<'r, PgRow> + Send + Unpin,
    {
        ...
        sql.push(T::table_name()).push(" SET (");

        let mut sepr = sql.separated(", ");
        T::base_columns().iter().for_each(|col| {
            sepr.push(col);
        });
        ...
        let mut sepr = sql.separated(", ");
        ((T::id_columns().len() + 1)..=T::columns().len()).for_each(|param| {
        ...
        let mut sepr = sql.separated(", ");
        T::id_columns().iter().for_each(|col| {
            sepr.push(col);
        });
        ...
        let mut sepr = sql.separated(", ");
        (1..=T::id_columns().len()).for_each(|param| {
            sepr.push(format!("${param}"));
        });
        ...
        T: Bindable + for<'r> FromRow<'r, PgRow> + Send + Unpin,
    {
        ...
        sql.push(T::table_name()).push(" WHERE (");
        ...
        T: Bindable + for<'r> FromRow<'r, PgRow> + Send + Unpin,
    {
        ...
        if let Some(some_id_s) = id_s {
            ...
            let mut sepr = sql.separated(", ");
            some_id_s.1.iter().for_each(|param| {
...
impl LiteInterface {
    pub async fn new() -> Result<LiteInterface, sqlx::Error> {
        ...
        let opt: SqliteConnectOptions = sqlite::SqliteConnectOptions::new()
            .filename(LOGS_PTH_STR)
        ...
        I: serde::Serialize,
    {
        ...
        let sql3: &'static str =
            "INSERT INTO logs (request, response, err_msg) VALUES (json($1), json($2), $3)";
