...
pub async fn get_group_member(Path(group_id) : Path<Uuid>, Path(member_id) : Path<Uuid>) -> (StatusCode, Json<Response<Vec<GroupMember>>>) {
    let inf = get_db_interface().await;

    println!("{}", group_id);
    println!("{}", member_id);

    return match inf.select::<Uuid, GroupMember>(Some((&GroupMember::id_columns(), &vec![group_id, member_id]))).await {
